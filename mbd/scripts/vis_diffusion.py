...
Hsample = 50
plot_interval = 10
# # Limit the number of trajectories to visualize (reduce from 99 to 5)
# max_trajectories = 1
path = f"{mbd.__path__[0]}/../results/{env_name}"
mu_0ts = jnp.load(f"{path}/mu_0ts.npy")
# print(f"Original trajectories: {mu_0ts.shape[0]}, using only {max_trajectories}")
# # Only use the first few trajectories instead of all 99
# mu_0ts = mu_0ts[:max_trajectories]
rng = jax.random.PRNGKey(0)
mu_0_random = jax.random.normal(rng, (Hsample, env.action_size))
mu_0ts = jnp.concatenate([mu_0_random[None], mu_0ts], axis=0)
# print(f"Total trajectories to visualize: {mu_0ts.shape[0]}")
...
print(rollouts)
json_file = dumps(env.sys, rollouts)

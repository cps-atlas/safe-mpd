...
def get_env(env_name: str, case: str = "parking", dt=0.2, H=50, motion_preference=0, 
            ...
            a_max=2.0, omega_max=1.0):
    if env_name == "car2d":
        return Car2d()
    elif env_name == "tt2d":
        return TractorTrailer2d(case=case, dt=dt, H=H, motion_preference=motion_preference, 
                               collision_penalty=collision_penalty, enable_gated_rollout_collision=enable_gated_rollout_collision,
                               hitch_penalty=hitch_penalty, enable_gated_rollout_hitch=enable_gated_rollout_hitch,
                               enable_projection=enable_projection, enable_guidance=enable_guidance,
        ...
        return AccTractorTrailer2d(case=case, dt=dt, H=H, motion_preference=motion_preference, 
                                  collision_penalty=collision_penalty, enable_gated_rollout_collision=enable_gated_rollout_collision,
                                  hitch_penalty=hitch_penalty, enable_gated_rollout_hitch=enable_gated_rollout_hitch,
                                  enable_projection=enable_projection, enable_guidance=enable_guidance,
                                  reward_threshold=reward_threshold, ref_reward_threshold=ref_reward_threshold,
